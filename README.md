...
| `brand` | `iPhone` | `Redmi` |
| `model` | `iPhone 15` | `Redmi K70` |
| `resolution` | `393*852` | `1080*2400` |
...
1. **学习用途**：本脚本仅供个人学习研究与自动化自己的账号使用，不得用于任何商业用途、批量操作或谋取利益。
2. **账号自负**：使用者需对自身账号的全部行为负责，因使用本脚本导致的账号封禁、积分清零、法律纠纷等后果，作者不承担任何责任。
3. **遵守协议**：使用本脚本即表示你已阅读并同意极核 APP 的用户协议，不得利用脚本从事违反平台规则的活动。
4. **禁止滥用**：禁止将本脚本用于代签、多账号批量签到、刷积分等灰产行为，违者自行承担法律责任。
5. **风险提示**：自动签到属于模拟客户端行为，存在被平台检测并处罚的风险，请谨慎使用，建议设置合理的签到时间间隔。
6. **停止使用**：若平台方要求停止使用或修改本脚本，请立即配合并删除相关文件。
